...
- apc_input:
  ...
  `list` object. A list containing the following model inputs in
  age-period-cohort notation: `J` (`integer`) Run-off triangle
  dimension. `eta` (`numeric`) Expected time-to-event in the cell. I.e.,
  lost exposure. `diagonal` (`numeric`) Cumulative payments last
  diagonal. `hazard.model` (`character`), hazard model specified from
  the user. Set to `user.specific` when a custom model is passed.
