...
list.of.datasets <- list(
  ...
  ukmotor = UKMotor,
  usapaid=USAApaid,
  aus1=incr2cum(austri1autoBI7895$paid),
  fre4b=fretri4auto9403$body$paid,
  swiss1=swtri1auto$paid,
  usa1=usautotri9504,
  usa2=ustri1fire$paid, 
...
```{r bake off 2, message=TRUE, warning=TRUE, include=FALSE}
...
```{r call bake off, message=TRUE, warning=TRUE, include=FALSE}
...
As a further analysis, the practitioner may be interested in inspecting the behavior of the age parameter across different models for different data sets. As extensively noticed in the paper we can appreciate a decreasing behavior.

## a models 

In a first place we provide the code to perform the graphical comparison for the age models.

```{r thirty plots ax, eval=FALSE}
...
This section provides the code to perform the same analyisis for the apc models.

```{r thirty plots ax apc, message=FALSE, warning=FALSE, include=FALSE}
...
```{r models ranking split, echo=FALSE}
# models ranking
...
```{r models ranking 1, include=FALSE}
...
```{r models ranking 2, include=FALSE}
...
```{r ranks counts by model, include=FALSE}
library(dplyr)
