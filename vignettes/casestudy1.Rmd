...
* We replicate the Mack chain-ladder under within our modeling framework. 
As shown in the paper, by using the `fchl` approach the resulting model is saving some parameters with respect to the standard GLM approach.
...
Consider the dataset we chose for this tutorial. 
...
In ... we showed that there is a one-to-one correspondence between the age-period representation and run-off triangles.
...
# Replicate the chain-ladder with the `fchl` package

Our package is an out of the box set of tools to compute the claims reserve.
We now show how to replicate the Mack chain-ladder.
...
#AMONG OURS
#sifa.gtpl is a case of good improvement
#amases.gtpl is a case of good improvement

#bz is good for showing apc

# taylor-ashe
# VNJ almost good
# auto$CommercialAutoPaid is better than TA
# AutoBI$AutoBIPaid is better than TA and previous auto
# MCLpaid is very good
# MedMal$MedMalPaid is very good
# incr2cum(austri1autoBI7895$paid) you still have trends but improved ones
# ustri1fire$paid is good


```

Starting from the results in ... we showed how to replicate the chain-ladder with an age model.
